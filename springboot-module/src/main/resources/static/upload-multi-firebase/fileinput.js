    ...
    FileInput.prototype = {
        ...
        _initTemplateDefaults: function () {
            ...
            tActionDelete = '<button type="button" class="kv-file-remove {removeClass}" ' +
                'title="{removeTitle}" {Url}{dataKey}>{removeIcon}</button>\n';
        ...
        upload: function () {
            /** Customized by dieu **/
            var self = this, files = self.getFileStack();
            $('#divUploading').attr('hidden', false);
            $('.fileinput-upload-button').attr('disabled', true);
            $('#divTitle').text(' ');
            $('#divUploading').attr('hidden', false);
            for (var i = 0; i < files.length; i++) {
                uploadSimpleFile(files[i], i, files.length);
            }
